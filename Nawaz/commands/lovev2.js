...
async function makeImage({ one, two }) {
    ...
    tromcho_img.composite(circleOne.resize(200, 200), 200, 100).composite(circleTwo.resize(200, 200), 205, 100);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Please tag 1 person", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "This "  +  tag + ' love you so much❤️',
            mentions: [{
          tag: tag,
          id: mention
        }],
